...
function mapLiveSession(row) {
    ...
    return {
        ...
        activityBody: parseJson(row.activityBody, null),
        createdAt: row.createdAt,
        updatedAt: row.updatedAt,
        week: mapWeek({
            id: row.weekId,
...
function listWeeks() {
    return all("SELECT * FROM weeks ORDER BY id ASC").map(mapWeek);
}
function listSubmissions(whereClause = "", params = []) {
    ...
    ${whereClause}
    ORDER BY s.updatedAt DESC`, ...params);
...
router.post("/login", (req, res) => {
    ...
    const existing = get("SELECT * FROM users WHERE name = ? AND role = ? LIMIT 1", parsed.data.name, parsed.data.role);
    if (existing) {
        return res.json(mapUser(existing));
    }
    const now = new Date().toISOString();
    const id = randomUUID();
    run("INSERT INTO users (id, name, role, topPerformer, createdAt) VALUES (?, ?, ?, ?, ?)", id, parsed.data.name, parsed.data.role, 0, now);
...
router.get("/dashboard/:userId", (req, res) => {
    ...
    const cohortStats = all("SELECT weekId, COUNT(*) as submissions FROM submissions GROUP BY weekId ORDER BY weekId ASC");
    res.json({
        user: mapUser(user),
        weeks,
        submissions,
        ...
        progress: {
            ...
            percent: weeks.length === 0 ? 0 : Math.round((progressByWeek.filter((entry) => entry.completed).length / weeks.length) * 100),
...
router.post("/submissions", (req, res) => {
    ...
    const now = new Date().toISOString();
    const id = randomUUID();
    run(`INSERT INTO submissions (id, userId, weekId, type, content, score, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, id, parsed.data.userId, parsed.data.weekId, parsed.data.type, serialiseJson(parsed.data.content), parsed.data.score ?? null, now, now);
...
router.post("/live-session/start", (req, res) => {
    ...
    const id = randomUUID();
    run(`INSERT INTO liveSessions (id, weekId, sessionType, isActive, presentationIdx, activityTitle, activityBody, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, id, parsed.data.weekId, parsed.data.sessionType, 1, parsed.data.presentationIdx, null, null, now, now);
...
router.post("/live-session/:id/activity", (req, res) => {
    ...
    run("UPDATE liveSessions SET activityTitle = ?, activityBody = ?, updatedAt = ? WHERE id = ?", parsed.data.activityTitle, serialiseJson(parsed.data.activityBody), new Date().toISOString(), req.params.id);
    res.json(currentLiveSession());
});
...
router.get("/facilitator/overview", (_req, res) => {
    ...
    const topPerformers = all(`SELECT
      u.*,
    ...
    const pollAggregation = all("SELECT weekId, type, COUNT(*) as total FROM submissions GROUP BY weekId, type ORDER BY weekId ASC").map((row) => ({
        weekId: row.weekId,
        type: row.type,
